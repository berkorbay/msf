...
GARCH_DUAN_MC_Q<-function(days_per_year,time_step,ttm,r_annualized,K,S0,m_length,n_length,garch_parameters,tao=90,constant_volatility=FALSE){
	...
	q1_star<-log((tao-min(tao,ttm))/tao*h_1+min(tao,ttm)/tao*h_star)
...
#	transition_matrix<-matrix(0,ncol=m_length*max(1,n_length),nrow=m_length*max(1,n_length))
	price_tmat<-matrix(logprice_p,ncol=m_length*n_length,nrow=m_length*n_length)
	if(constant_volatility){
		vol_filter<-1
	}else{
		next_vols<-log(garch_omega+garch_beta*exp(state_matrix[,"var"])+garch_alpha*(t(price_tmat)-price_tmat+0.5*(exp(state_matrix[,"var"])-
			h_star)-(garch_theta+garch_lambda)*exp(state_matrix[,"var"]/2))^2)
		vol_filter<-t((t(next_vols)<sort(rep(logvar_d[-1],m_length)))*(t(next_vols)>sort(rep(logvar_d[-(n_length+1)],m_length))))
	}

	ck_pi<-t(matrix(logprice_c,ncol=m_length*n_length,nrow=(m_length+1)*n_length))-logprice_p
#	q_l<-sort(rep(logvar_q,(m_length+1)))
#	l_ijk_mat<-pnorm(t((t(ck_pi)+0.5*(exp(q_l)-h_star))/sqrt(exp(q_l))))
...
#	transition_probabilities<-cdf_values[,-1]-cdf_values[,-(m_length+1)]
	transition_probabilities<-l_ijk_mat[,-((0:(n_length-1))*(m_length+1)+1)]-l_ijk_mat[,-(1:n_length)*(m_length+1)]
...
#garch_pars<-list(omega=(0.2)^2*(30/365),beta=0.8,alpha=0.1,theta=0.3,lambda=0.2)

start_time<-Sys.time()
print(start_time)
GARCH_DUAN_MC_Q(days_per_year=365,time_step=1,ttm=270,r_annualized=0.05,K=50,S0=50,m_length=357,n_length=51,tao=90,garch_parameters=garch_pars,constant_volatility=FALSE)
Sys.time()-start_time
